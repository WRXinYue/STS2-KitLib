...
internal static class KitLibSatelliteModuleSettingsUi {
    ...
    static ModuleRowBinding CreateModuleRow(
        ...
        if (alwaysOn) {
            ...
            DevModeFormChrome.ApplyToggle((CheckBox)toggle);
            toggle.Toggled += on => {
    ...
    static void RefreshModuleRows(IReadOnlyList<ModuleRowBinding> bindings) {
        ...
        foreach (var binding in bindings) {
            ...
            if (binding.Toggle != null) {
                ...
                var requiresPanel = string.Equals(binding.ModuleId, KitLibModuleIds.Dev, StringComparison.OrdinalIgnoreCase)
                    || string.Equals(binding.ModuleId, KitLibModuleIds.Ai, StringComparison.OrdinalIgnoreCase);
