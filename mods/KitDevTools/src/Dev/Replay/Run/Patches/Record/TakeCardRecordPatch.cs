...
public static class TakeCardRecordPatch {
    ...
    public static void OnAlternatePrefix(NCardRewardSelectionScreen __instance, int index) {
        ...
        var extraOptions = ExtraOptionsField?.GetValue(__instance) as IReadOnlyList<CardRewardAlternative>;
        if (extraOptions == null || index < 0 || index >= extraOptions.Count) return;

        var afterSelected = extraOptions[index].AfterSelected;
