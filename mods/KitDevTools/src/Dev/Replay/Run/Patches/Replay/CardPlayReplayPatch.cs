...
public static class CardPlayReplayPatch {
    ...
    internal static void InvalidateStaleTimers() {
        PlayerActionBuffer.LogToDevConsole($"[KitLib.RunReplay] InvalidateStaleTimers: gen {_battleGeneration} → {_battleGeneration + 1}");
        _battleGeneration++;
    ...
    private static void OnTurnStarted(CombatState combatState) {
        ...
        PlayerActionBuffer.LogToDevConsole(
            $"[KitLib.RunReplay] >>> TurnStarted round={combatState.RoundNumber}" +
        ...
        if (_waitingForEffects || _dispatching) {
            PlayerActionBuffer.LogToDevConsole("[KitLib.RunReplay] TurnStarted: dispatch chain already active, skipping.");
            return;
        }
    ...
    private static void OnTurnEnded(CombatState combatState) {
        PlayerActionBuffer.LogToDevConsole(
            $"[KitLib.RunReplay] >>> TurnEnded round={combatState.RoundNumber}" +
    ...
    private static void OnAfterActionExecuted(GameAction action) {
        ...
        if (!_dispatching && action is DiscardPotionGameAction) {
            PlayerActionBuffer.LogToDevConsole("[KitLib.RunReplay] AfterActionExecuted: DiscardPotionGameAction completed outside combat — resuming rewards.");
            ReplayDispatcher.DispatchNow();
            return;
        }
        ...
        if (action is PlayCardAction playCard) {
            ...
            PlayerActionBuffer.LogToDevConsole($"[KitLib.RunReplay] AfterActionExecuted: DiscardPotionGameAction completed ({discardPotion}).");
            WaitForEffectsThenDispatch();
        }
