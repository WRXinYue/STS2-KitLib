...
using KitLib.Replay.Patches.Replay;
...
public static class ReplayDispatcher {
    ...
    public static List<ReplayCommand> GetAvailableCommands() {
        ...
        foreach (var type in types) {
            ...
            else if (type == typeof(ChooseRelicCommand)) {
                var screen = ReplayState.ActiveRelicScreen;
                if (screen != null && GodotObject.IsInstanceValid(screen) && screen.IsInsideTree()) {
                    int n = ChooseRelicCommand.ListChoices(screen).Count;
                    for (int i = 0; i < n; i++)
                        commands.Add(new ChooseRelicCommand(i));
                }
            }
    ...
    private static HashSet<Type> GetDispatchableTypes() {
        ...
        if (ReplayState.ActiveRelicScreen != null
            && GodotObject.IsInstanceValid(ReplayState.ActiveRelicScreen)
            && ReplayState.ActiveRelicScreen.IsInsideTree())
            types.Add(typeof(ChooseRelicCommand));
        ...
        if (currentRoom is EventRoom
            ...
            // Ancient starting bonus (Neow) uses EventSynchronizer but is not an EventRoom.
    ...
    /// <summary>True while fast-forwarding to a clicked timeline room.</summary>
    public static bool IsSeeking { get; private set; }

    public static void SetSeeking(bool seeking) {
        IsSeeking = seeking;
        RefreshPacing();
        if (seeking)
            GameSpeed = 8f;
    ...
    public static void RefreshPacing() {
        if (IsSeeking) {
            _delayBetweenCommands = 0f;
            return;
        }
        _delayBetweenCommands = ReplayLiveMode.Enabled ? 0.12f : 0f;
    }

    static float AfterSuccessDelay =>
        IsSeeking ? 0f : ReplayLiveMode.Enabled ? 0.12f : 0f;
    /// <summary>
    ...
    private static void OnRoomEntered() {
        ...
        GD.Print($"[KitLib.RunReplay] OnRoomEntered — room={room?.GetType().Name ?? "null"} active={ReplayEngine.IsActive}");
    ...
    private static void OnRoomExited() {
        var room = GetCurrentRoom();
        GD.Print($"[KitLib.RunReplay] OnRoomExited — room={room?.GetType().Name ?? "null"} active={ReplayEngine.IsActive}");
    ...
    internal static void TryDispatch([System.Runtime.CompilerServices.CallerMemberName] string? caller = null) {
        ...
        GD.Print($"[KitLib.RunReplay] TryDispatch caller={caller} active={ReplayEngine.IsActive} paused={_paused} next={peekCmd?.GetType().Name ?? "null"}({peekCmd})");
        ...
        if (!dispatchable.Contains(cmd.GetType())) {
            ...
            GD.Print($"[KitLib.RunReplay] TryDispatch miss — cmd={cmd.GetType().Name} dispatchable=[{dispatchableNames}]");
            DiagnosticLog.Write("Dispatch",
        ...
        if (_dispatchInProgress && cmd == _lastDispatchedCmd) {
            GD.Print($"[KitLib.RunReplay] TryDispatch skip — already in progress ({cmd.GetType().Name})");
            return;
        }
    ...
    private static void ExecuteNext() {
        ReplayEngine.PeekNext(out ReplayCommand? peekCmd);
        GD.Print($"[KitLib.RunReplay] ExecuteNext entry — active={ReplayEngine.IsActive} paused={_paused} next={peekCmd?.GetType().Name ?? "null"}({peekCmd})");
        ...
        if (cmd is not ChooseEventOptionCommand and not SelectCardFromScreenCommand
            && ReplayLiveMode.ConsumeThink(cmd, out int thinkMs)) {
            _dispatchInProgress = false;
            _lastDispatchedCmd = null;
            int thinkGen = ++_dispatchGeneration;
            NGame.Instance?.GetTree()?.CreateTimer(thinkMs / 1000f).Connect(
                "timeout", Callable.From(() => {
                    if (_dispatchGeneration == thinkGen)
                        TryDispatch();
                }));
            return;
        }
