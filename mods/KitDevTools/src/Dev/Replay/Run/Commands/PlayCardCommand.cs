...
public class PlayCardCommand : ReplayCommand {
    ...
    public override ExecuteResult Execute() {
        ...
        try {
            ...
            PlayerActionBuffer.LogDispatcher($"[KitLib.RunReplay] TryPlayNextCard: resolved card '{card}' from index {CombatCardIndex}.");
        }
        catch (Exception ex) {
            PlayerActionBuffer.LogToDevConsole(
                $"[KitLib.RunReplay] TryPlayNextCard: GetCard({CombatCardIndex}) threw {ex.GetType().Name}: {ex.Message}");
        ...
        if (TargetId.HasValue) {
            ...
            PlayerActionBuffer.LogDispatcher($"[KitLib.RunReplay] TryPlayNextCard: resolved target id={TargetId} → {(target == null ? "null" : target.ToString())}.");
        }
