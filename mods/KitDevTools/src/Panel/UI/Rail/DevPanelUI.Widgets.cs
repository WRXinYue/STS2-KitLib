...
internal static partial class DevPanelUI {
    ...
    internal static Control CreateCheatSlider(string label, string? tooltip, float min, float max, float step,
    ...
    internal static Control CreateCheatNumberEdit(string label, int min, int max, Func<int> getter, Action<int> setter) {
        var row = new HBoxContainer();
        row.AddThemeConstantOverride("separation", 4);
        row.CustomMinimumSize = new Vector2(0, 30);
        var lbl = new Label { Text = label, SizeFlagsHorizontal = Control.SizeFlags.ExpandFill, ClipText = true };
    ...
    internal static Control CreateStatLockRow(string label, int min, int max,
