...
双开时 **不会** 自动套用 LAN preset；开局后在各窗口 DevPanel → AI 托管 手动点击：

| 端 | DevPanel 操作 | log 关键词 |
| --- | --- | --- |
| **主机** | 「Apply LAN Host-Drive Preset」 | `LAN host preset applied` |
| **客机** | 「Apply LAN Client AFK Preset」 | `AFK client enabled` |

AFK 开关是 **per-process**（`KitLibInstance.SessionLan`）。
:::
...
**半配置会 desync：** 只开客机 AFK、主机未开 LAN preset 时，1000 无人代打且 coop 回合门控易乱。双开时两边都要手动点。
:::
...
双开 RunManager.Launch
  → （无自动 preset）DevPanel 手动 ApplyLanHostPreset / ApplyLanClientPreset
...
| 路径 | 职责 |
| --- | --- |
| `src/Multiplayer/LanTest/DualInstanceTestBootstrap.cs` | 双开 cheat / DevMode 默认 |
...
Select-String -Path "$env:APPDATA\SlayTheSpire2\logs\godot.log" `
  -Pattern "LAN host preset|AFK client enabled|GameLoop|Enqueued end turn|StateDivergence|Ready end turn \(live ENet\)"
```

- 有 `LAN host preset applied`
...
| 6 | phase 1 vs 2 | 主机未 preset + 客机单方面 ready enemy turn + `ClearInFlight(OwnerId)` 清错 | 双开需手动 preset；移除客机 enemy-turn patch；`ResolvePlayerNetId` |
...
1. 两边重启并加载最新 DLL。
2. 主机 log：`LAN host preset applied`（DevPanel 手动点击后）。
