...
敌人阶段 ready:
  ...
  live ENet host-drive → 主机 RequestEnqueue(ReadyToBeginEnemyTurnAction for peer) + owner-routed patch
  客机 AFK EndTurnPhaseOne 兜底 TrySignalReadyToBeginEnemyTurn；允许本地 ReadyToBeginEnemyTurn RequestEnqueue
...
| Enemy-turn ready | host SetReadyToBeginEnemyTurn(peer) | host RequestEnqueue(ReadyToBeginEnemyTurn for peer) + client AFK 兜底 |
...
- 第一回合结束应有 `Enqueued ready-to-begin-enemy-turn netId=1000`（host）或 `AFK client ready-to-begin-enemy-turn netId=1000`（client 兜底）
...
| 9 | 210300 第一回合后软锁（无 `After enemy turn start`） | live ENet 跳过 P1000 `ReadyToBeginEnemyTurn`；双端 AI EndTurn → `EndTurnPhaseOne` 无 `NotPlayPhase`；AFK 拦截 vanilla ready enqueue | host `RequestEnqueue(ReadyToBeginEnemyTurn for peer)`；AFK `EndTurnPhaseOne` 兜底 + 放行 `ReadyToBeginEnemyTurnAction` |

### #9 详情（2026-05-24 ~17:50，pid 210300）
...
- 修复：与 end-turn 对称，host owner-routed enqueue P1000 ready；client AFK 在 `EndTurnPhaseOne` 补发
