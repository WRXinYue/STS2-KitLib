...
敌人阶段 ready:
  ...
  phantom 无独立 client → 主机 ReadySimulatedPeersToBeginEnemyTurn (SetReadyToBeginEnemyTurn)
  live ENet **跳过** host-only SetReadyToBeginEnemyTurn（避免重复 / 单方面 phase 2）
  客机 **不** 单方面 SetReadyToBeginEnemyTurn
...
| Enemy-turn ready | host SetReadyToBeginEnemyTurn(peer) | client 自行 enqueue GetMe |
...
- **无** `AFK client ready-to-begin-enemy-turn`
