...
敌人阶段 ready:
  ...
  live ENet host-drive → 客机 AFK 在 P1 Ready 执行后 RequestEnqueue(Ready for self)；host 不 enqueue P1000 Ready
...
- **每个 round 先** host/client 均出现 `After player turn phase one end`（id N），**再** client `AFK client ready-to-begin-enemy-turn` 或 host `RequestEnqueueActionMessage.*ReadyToBegin.*1000`
...
### #11 详情（2026-05-24 ~18:03，pid 214292/214288）
