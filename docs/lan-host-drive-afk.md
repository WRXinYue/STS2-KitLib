...
敌人阶段 ready:
  ...
  客机 AFK 拦截全部本地 RequestEnqueue（含 Ready）；仅接受 host 广播的 ActionEnqueuedMessage
...
- **每个 round 至多 1 条** `Enqueued ready-to-begin-enemy-turn netId=1000`（仅 host）
- **无** 客机 `Requesting to enqueue.*ReadyToBegin.*1000` 或 `AFK client ready-to-begin-enemy-turn`
...
| 10 | 212268/210636 checksum 17；`EndPlayerTurnPhaseTwo.*Enemy` | #9 host + client 双路径各 enqueue P1000 Ready → 重复 action、phase 1/2 分叉 | 移除 client `TrySignalReadyToBeginEnemyTurn`；AFK 拦截含 Ready 的全部本地 RequestEnqueue |
...
### #10 详情（2026-05-24 ~18:xx，pid 212268/210636）
