...
public static class ModVariantBootstrap {
    ...
    static readonly string[] OptionalKitLibHostDeps = [
    ...
    public static void Initialize(ModVariantBootstrapOptions? options) {
        ...
        if (Directory.Exists(libRoot)) {
            ...
            if (string.Equals(modId, KitLibModFolderName, StringComparison.OrdinalIgnoreCase)
                || string.Equals(implFile, KitLibCoreFileName, StringComparison.OrdinalIgnoreCase))
    ...
    private static void EnsureHostDependencies(Assembly hostAssembly) {
        ...
        foreach (var fileName in OptionalKitLibHostDeps) {
            var simpleName = Path.GetFileNameWithoutExtension(fileName);
            if (FindLoaded(alc, simpleName) != null)
                continue;
            var path = FindHostDependencyPath(fileName, kitLibDir, hostDir);
    ...
    static string? FindHostDependencyPath(string fileName, string kitLibDir, string hostDir) {
        foreach (var dir in EnumerateHostDependencyDirectories(kitLibDir, hostDir)) {
    ...
    static IEnumerable<string> EnumerateHostDependencyDirectories(string kitLibDir, string hostDir) {
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        IEnumerable<string> YieldDir(string? dir) {
        ...
        foreach (var dir in YieldDir(picked))
            yield return dir;

        if (picked is null) {
            foreach (var dir in YieldDir(kitLibDir))
                yield return dir;
        }

        foreach (var dir in YieldDir(hostDir))
            yield return dir;
    }
