...
通过 [Model Context Protocol](https://modelcontextprotocol.io) 将 run 状态与操作暴露给任意 MCP 客户端（Claude Desktop、IDE MCP 插件等）。Mod 内 HTTP 桥接默认监听 **9877**；`tools/DevMode.Mcp` 中的 stdio 代理把 MCP 消息转发到该端口。

**前提：** 《杀戮尖塔 2》已运行且 **DevMode** 已加载（先开游戏，或保持游戏运行后再连接 MCP 客户端）。

### 工具

- **`get_game_state`** — 当前 run 快照（生命、金币、牌组、战斗、敌人等）
- **`combat_action`** — 出牌、结束回合、使用药水
...
详见 **[tools/DevMode.Mcp/README.md](./tools/DevMode.Mcp/README.md)**
...
先编译代理（或使用下方平台启动脚本自动编译）：

```bash
dotnet build tools/DevMode.Mcp/DevMode.Mcp.csproj -c Release
```

把下面的 `devmode` 块粘贴进现有 MCP 客户端配置（与已有 `mcpServers` 条目合并）。
...
**已 publish 的代理**（`dotnet publish` 后；按实际路径修改 `command`）：
...
macOS / Linux 将 `command` 指向 publish 出的可执行文件（无 `.exe`）。若改端口，代理的 `--port` 须与 mod 内 `McpConfig.Port` 一致（改端口需重新编译 DevMode）。
