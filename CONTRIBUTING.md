...
- Before opening a PR: **`dotnet build KitLib.sln`** (or `make build`), then **`make check`** (`format-check` + `lint-scripts`). If you changed C# formatting, run **`make format`** first. With hooks installed, staged commits run the same checks automatically.
