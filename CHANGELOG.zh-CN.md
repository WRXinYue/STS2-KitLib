...
- **游戏操作 API** — 内容 mod 可通过 `KitLibGameOps` 读取 lean 局面并驱动玩家 UI（出牌、选图、领奖励等）。不需要安装 KitAI。
...
- **Android 支持** — Android 上 KitLib 模块默认尝试加载；无法加载的模块会自动跳过，不影响其余功能。
...
- **设置** — 切换 Dark / OLED / Light 主题后，设置面板不再卡住无法关闭。感谢 [@minatowolf](https://github.com/minatowolf) 反馈。
...
- **AI Host 面板** — Dev Mode → AI Host 显示当前同伴数量与单人同伴模式提示。
- **Dev Mode 轨道展开** — Mod 面板 → KitLib → 常规：**轨道展开方式** — **悬停标签展开**（默认）悬停左侧小标签展开轨道；**侧边悬停展开**悬停屏幕左缘展开且隐藏小标签。
- **地图预测** — Dev Mode → 作弊 → 游戏：**地图预测**（默认关闭）。开启作弊模式后，悬停未访问的地图节点可预览房间类型、遭遇战与事件；`?` 节点按与游戏相同的随机规则推算。
- **开发者控制台（浏览器）** — 统一开发面板 `http://127.0.0.1:9878`，含 **日志** 与 **战斗** 两个标签：实时日志流、与局内日志查看器筛选同步、战斗事件时间线（每回合状态、搜索、类型筛选）及 JSON 导出。支持中英文。
- **局内日志 → 开发者面板** — 日志查看器新增 **开发者面板** 按钮；开启「与游戏同步」时筛选规则与浏览器面板保持一致。
- **原版错误弹窗** — 带「反馈 Bug」的弹窗在「反馈 Bug」下方新增 **日志导出 (KitLib)** 按钮，可一键导出诊断 ZIP。
- **KitLib 快捷键** — 支持绑定组合键（如 Ctrl+Shift+M）。
...
- **战斗统计多视图** — 移除概览、按卡牌、承伤、按回合图表、扩展统计与整局累计；时间线在浏览器查看器中展示。
- **战斗统计右侧轨** — 移除局内右侧事件浮层。
...
- **Dev Mode 轨道（悬停标签展开）** — 修复悬停小标签时轨道反复展开与收起的问题。
- **地图预测** — 已预测为商店、宝箱或休息点的 `?` 节点，不再在正确的房间类型标题下显示「无法预测」。
- **Dev Mode 侧栏（异常后）** — 过渡动画中途发生游戏错误（如房间加载失败）后，左侧轨道不再出现能悬停展开但图标点不动的情况。
- **Dev Mode 面板（原版弹窗）** — 原版全屏对话框弹出时（如存档损坏无法加载），侧栏滑出的面板内容可再次正常点击；此前只有侧栏能响应。
- **KitLib 快捷键** — 修复快捷键绑定问题。
...
- **Mod 面板** — **Ctrl+Shift+M** 可在主菜单或局内打开/关闭面板；可在 KitLib **快捷键** 中改绑。
